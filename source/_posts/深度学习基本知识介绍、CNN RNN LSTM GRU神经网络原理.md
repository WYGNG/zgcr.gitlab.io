...
L1正则化的最优化问题等价于:
$$
\min _{w} L(w)=\min _{w}\left(E_{D}(w)+\lambda \sum_{i=1}^{n} w_{i} |\right)
$$
可写为凸优化问题:
$$
\left\{\begin{array}{l}{\min E_{D}(w)} \\\\ {\sum_{i=1}^{n}\left|w_{i}\right| \leq C}\end{array}\right.
$$
...
L2正则化的最优化问题等价于:
$$
\min _{w} L(w)=\min _{w}\left(E_{D}(w)+\lambda \sum_{i=1}^{n} w_{i}^{2}\right)
$$
可写为凸优化问题:
$$
\left\{\begin{array}{l}{\min _{w} E_{D}(w)} \\\\ {\sum_{i=1}^{n} w_{i}^{2} \leq C}\end{array}\right.
$$
其中C与正则化参数λ成反比关系。也就是在一个限定的区域内找到:
$$
\min E_{D}(w)
